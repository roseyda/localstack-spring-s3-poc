  ...
  public AmazonS3 getAmazonS3() {
    AmazonS3ClientBuilder builder = AmazonS3ClientBuilder.standard()
        .withEndpointConfiguration(
            new AwsClientBuilder.EndpointConfiguration(S3_SERVICE_ENDPOINT, S3_SERVICE_REGION))
        .withPathStyleAccessEnabled(true);

    if (Arrays.stream(environment.getActiveProfiles())
        .anyMatch(profile -> LOCAKSTACK_PROFIL.equalsIgnoreCase(profile))) {
      builder.withClientConfiguration(new ClientConfiguration().withNonProxyHosts(NON_PROXY_HOST));
    }
